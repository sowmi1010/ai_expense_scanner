    ...
    try {
      ...
      if (image == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text(AppStrings.scanNoImageSelected)),
        );
        return;
      }

      AppRoutes.toReceiptPreview(context, imagePath: image.path);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${AppStrings.scanImportFailed}: $e')),
      );
    } finally {
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: OutlinedButton.icon(
                                    ...
                                    label: Text(
                                      _picking
                                          ? AppStrings.scanLoading
                                          : AppStrings.scanImportGallery,
                                    ),
                                  ),
                                ),
                                const SizedBox(height: 10),
    ...
    return IgnorePointer(
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: color,
          boxShadow: [
            BoxShadow(blurRadius: 70, spreadRadius: 18, color: color),
          ],
        ),
      ),
    );
